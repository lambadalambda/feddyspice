...
- README now reflects current capabilities (streaming, media, interactions, config).
...
- Lists API endpoints (`GET/POST/PUT/DELETE /api/v1/lists`, `GET/POST/DELETE /api/v1/lists/:id/accounts`).
- Filters API endpoints (`GET/POST/PUT/DELETE /api/v1/filters`).
...
- Status content now linkifies known `@mentions` and returns a populated `mentions` array (Elk thread + hover UX).
- Status interaction endpoints now persist relationship state and expose booleans on status payloads (`favourited`, `reblogged`, `bookmarked`, `pinned`, `muted`), including `pin`/`unpin`/`mute`/`unmute` (`POST /api/v1/statuses/:id/{favourite,unfavourite,reblog,unreblog,bookmark,unbookmark,pin,unpin,mute,unmute}`).
...
- HTML form same-origin checks validate against configured `FEDDYSPICE_DOMAIN` + `FEDDYSPICE_SCHEME` (works behind reverse proxies like Coolify).
...
- Followers-only ActivityPub Notes are now stored as `visibility=private` (not `direct`); the home timeline excludes `direct` statuses and only includes remote statuses from accepted follows (DMs stay in `/api/v1/timelines/direct`).
